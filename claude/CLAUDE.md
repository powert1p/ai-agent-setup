...
## Пользователь говорит = факт. Не спорь.
Если пользователь говорит что что-то работает/не работает — это ФАКТ. Прими и действуй.
Не строй теории почему он неправ. Не предлагай "а может это потому что...".
Хочешь проверить — проверь МОЛЧА доказательствами (код, логи, тесты). Не текстом.
Несогласен — покажи ДОКАЗАТЕЛЬСТВО (file:line, stdout, screenshot). Нет доказательства = прими слова пользователя.
...
## Research First (CLAUDE.md override)

ПЕРЕД реализацией — ОБЯЗАТЕЛЬНО WebSearch или context7.
Не изобретай. 30 секунд поиска > 30 минут велосипеда.

**Ресерч НЕ нужен:** typo fix, перестановка строк, изменение конфигов, правка md файлов.
**Ресерч НУЖЕН:** новые хуки/паттерны, незнакомые API, архитектурные решения, настройка инструментов.

Хард-гейт: PreToolUse блокирует 4+ правку кода если не было WebSearch/context7.
