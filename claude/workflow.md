...
**Brainstorming = ВСЕГДА. На ЛЮБОЕ сообщение. Без исключений.**
...
**ПЕРВОЕ ДЕЙСТВИЕ на ЛЮБОЕ сообщение** = `Skill('brainstorming')`. Без исключений.
...
- Research (нетривиальные): WebSearch/context7 ПЕРЕД реализацией. Не изобретай велосипеды
- No-code orchestrator: оркестратор НЕ кодит, делегирует через Agent → implementer
